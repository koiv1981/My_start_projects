...
# функция очистки табло и присваивания 1 в
# переменную z при нажатии на кнопку "+"
def numb_plus():
    global a, z
    a = float(tablo.get())
...
# функция очистски табло и присваивания 2 в
# переменную z при нажатии на кнопку "-"
def numb_minus():
    global a, z
    a = float(tablo.get())
...
# функция очистки табло и присваивания 3
# в переменную z при нажатии на кнопку "*"
def numb_umn():
    global a, z
    a = float(tablo.get())
    tablo.delete(0, tk.END)
...
# функция очистки табло и присваивания 4
# в переменную z при нажатии на кнопку "/"
def numb_razd():
    global a, z
    a = float(tablo.get())
...
# функция очистки табло и присваивания 5
# в переменную z при нажатии на кнопку "^"
def numb_step():
    global a, z
    a = float(tablo.get())
...
# функция очистки табло и извлечения квадратного
# корня из введенного числа при нажатии на кнопку "√"
def numb_kor():
    global a
    a = float(tablo.get())
    tablo.delete(0, tk.END)
...
# основная функция расчета при нажатии кнопки "="
def numb_ravno():
    global a, b, z
    b = float(tablo.get())
    tablo.delete(0, tk.END)
    if z == 1:
        ...
        try:
            tablo.insert(tk.END, a*b)
            a = a * b
        except OverflowError:  # исключение при ошибке OverflowError
...
but0 = list("С√^")  # первый ряд кнопок
but1 = list("123+-")  # второй ряд кнопок
but2 = list("456*/")  # третий ряд кнопок
but3 = list("7890=")  # четвертый ряд кнопок
